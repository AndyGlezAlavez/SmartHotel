...
namespace SmartHotel.Domain.Entities
{
    ...
    public class Room : Entity
    {
        ...
        /// <returns></returns>
        public bool IsRentabled(DateTime startDate, DateTime finalDate)  //La función recibe las fecha de inicio y fin deseadas para efectuar una reserva (es llamada mientras se intenta crear un nuevo acuerdo de reserva por la entidad ´Agreement´).  
        {
            if (Smoke.Danger()) //Si la concentración de humo en la habitación solicitada supera el valor normal...
                ...
                return !Agreements.Any(a => startDate < a.FinalDate && finalDate > a.StartDate);  //...donde se recorre la lista de acuerdos de reservas(´Agreements´) asociados a la habitación. Se comparan las fechas donde la habitación ya está rentada (las de la lista) con las nuevas fechas en que se solicita la reserva (las que recibe esta función). Aquí hay 4 posibilidades...
                                                                                                  //Posibilidad 1: Que la nueva solicitud comience antes de que finalice una de las previstas (startDate < a.FinalDate) pero termine después del comienzo de esa prevista (finalDate > a.StartDate). Ej: SOLICITUD: 15/5-20/5, RESERVA ANTES CONFIRMADA: 18/5-21/5. Lo que implicaría que coincidieran iguales días de 2 reservaciones diferentes (18/5, 19/5 Y 20/5) para una misma habitación (NO PUEDE OCURRIR). Como las 2 condiciones se cumplen, la función ´Any´ devolverá TRUE pero la función ´IsRentabled´ devolverá lo contrario (FALSE), indicando de que NO ES POSIBLE rentar esa habitación para las fechas que se están recibiendo.
